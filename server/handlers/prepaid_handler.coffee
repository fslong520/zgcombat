...
PrepaidHandler = class PrepaidHandler extends Handler
  ...
  hasAccess: (req) ->
    req.method is 'GET' || req.user?.isAdmin() || req.user?.isTeacher()
  ...
  createPrepaidAPI: (req, res) ->
    ...
    # Allow teachers and admins to create course licenses
    return @sendForbiddenError(res) unless req.body.type in ['course', 'starter_license', 'subscription','terminal_subscription']
    ...
    if req.body.type in ['course', 'starter_license']
      properties.courseIDs = req.body.courseIDs or []
