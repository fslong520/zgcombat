  ...
  CertificatesView = class CertificatesView extends RootView {
    ...
    constructor (options, userID) {
      ...
      if (this.userID === me.id) {
        this.user = me
        if (utils.iszgcombat && !utils.showOzaria()) {
          this.setHero()
        }
      } else {
    ...
    calculateCourseStats () {
      ...
      this.sessions.forEach(session => {
        let left
        playtime += (left = session.get('playtime')) != null ? left : 0
      })
    ...
    calculateCampaignStats () {
      ...
      let linesOfCode = 0
      for (const session of Array.from((this.sessions != null ? this.sessions.models : undefined))) {
        let left
