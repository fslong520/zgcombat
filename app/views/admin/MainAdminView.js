        ...
        .then(model => {
          ...
          for (let index = 0; index < iterable.length; index++) {
            ...
            for (const level of Array.from(course.levels)) {
              ...
              for (const intro of Array.from(level.introContent != null ? level.introContent : [])) {
          ...
          for (const user of Array.from(users.models)) { userMap[user.id] = user }
          sessions = new LevelSessions()
          return Promise.resolve($.when(...Array.from(sessions.fetchForAllClassroomMembers(classroom) || [])))
        }).then(models => {
          for (const session of Array.from(sessions.models)) {
            var playtime
            if (!__guard__(session.get('state'), x => x.complete)) { continue }
            const levelID = session.get('level').original
            const userID = session.get('creator')
            ...
            if (session.get('contentPlaytimes')) {
              playtime = 0
              for (const content of Array.from(session.get('contentPlaytimes'))) { playtime += content.playtime != null ? content.playtime : 0 }
            } else {
          ...
          for (const session of Array.from(interactiveSessions.models)) {
          ...
          for (userID in userMap) {
            ...
            const row = [(left = user.get('name')) != null ? left : 'Anonymous']
            ...
            for (interactive of Array.from(courseInteractives)) {
              const attempts = userInteractiveAttemptMap[userID] != null ? userInteractiveAttemptMap[userID][interactive.interactiveID] : undefined
              if (attempts) {
                row.push(attempts)
              } else {
                row.push('Incomplete')
          ...
          for (const studentRow of Array.from(userRows)) {
