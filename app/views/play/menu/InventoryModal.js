...
module.exports = (InventoryModal = (function () {
  InventoryModal = class InventoryModal extends ModalView {
    ...
    onItemsLoaded () {
      ...
      // Drop ThangTypes that aren't actually equippable items (leaked in by the
      // server's items view) so we don't warn on them or render blank items.
      this.items.reset(this.items.filter(item => item.isRealItem()))
    ...
    getRenderData (context) {
      ...
      for (const slot in context.equipment) { const itemOriginal = context.equipment[slot]; context.equipment[slot] = this.items.findWhere({ original: itemOriginal }) }
    ...
    setUpDraggableEventsForAvailableEquipment () {
      ...
      return (() => {
        const result = []
        for (let availableItemEl of Array.from(this.$el.find('#unequipped .item'))) {
          availableItemEl = $(availableItemEl)
          if (availableItemEl.hasClass('locked') || availableItemEl.hasClass('restricted')) { continue }
          const dragHelper = availableItemEl.clone().addClass('draggable-item')
    ...
    setUpDraggableEventsForEquippedArea () {
      ...
      for (const itemSlot of Array.from(this.$el.find('.item-slot'))) {
        const slot = $(itemSlot).data('slot');
        ((slot, itemSlot) => {
          $(itemSlot).droppable({
    ...
    onUnequippedItemDoubleClick (e) {
      ...
      return _.defer(() => { this.justDoubleClicked = false })
    }
    ...
    getCurrentEquipmentConfig () {
      const config = {}
      for (const slot of Array.from(this.$el.find('.item-slot'))) {
        const slotName = $(slot).data('slot')
        const slotItemID = $(slot).find('.item').data('item-id')
        if (!slotItemID) { continue }
        const item = _.find(this.items.models, { id: slotItemID })
    ...
    updateLevelRequiredItems (equipment) {
      ...
      for (const slot in requiredGear) {
        ...
        if (items.length) {
          ...
          const bestOwnedItem = _.findLast(items, item => me.ownsItem(item))
          ...
          const itemModel = this.items.findWhere({ original: item })
          const availableSlotSelector = `#unequipped .item[data-item-id='${itemModel.id}']`
          this.highlightElement(availableSlotSelector, { delay: 500, sides: ['right'], rotation: Math.PI / 2 })
          const $itemEl = this.$el.find(availableSlotSelector).addClass('should-equip')
          this.$el.find(`#equipped div[data-slot='${slot}']`).addClass('should-equip')
          const itemOffsetTop = $itemEl[0] != null ? $itemEl[0].offsetTop : undefined
          if (itemOffsetTop) {
            const itemOffsetBottom = itemOffsetTop + $itemEl.outerHeight(true)
            const parentHeight = $itemEl.parent().height()
    ...
    updateConfig (callback, skipSessionSave) {
      ...
      const hero = this.selectedHero != null ? this.selectedHero.get('original') : undefined
      if (hero) {
      ...
      return (() => {
        ...
        for (let itemEl of Array.from(items)) {
          ...
          if ((itemEl.position().top < threshold) || forceLoadAll) {
            ...
            const item = this.items.get(itemEl.data('item-id'))
    ...
    onEquipmentChanged () {
      ...
      for (const slot in equipment) {
        const original = equipment[slot]
        const item = _.find(this.items.models, item => item.get('original') === original)
        const dollImages = item != null ? item.get('dollImages') : undefined
        if (!dollImages) { continue }
