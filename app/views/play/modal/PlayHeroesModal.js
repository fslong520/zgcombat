...
module.exports = (PlayHeroesModal = (function () {
  PlayHeroesModal = class PlayHeroesModal extends ModalView {
    ...
    onHeroesLoaded () {
      ...
      // Drop ThangTypes that aren't actually equipping heroes (e.g. pets/doodads
      // leaked in by the server's heroes view) so we don't warn or crash on them.
      this.heroes.reset(this.heroes.filter(hero => hero.isRealHero()))
    ...
    loadHero (hero, preloading) {
      ...
      if (poseImage) {
        ...
        // No posed portrait for this hero; keep the (possibly animated) canvas
        // instead of crashing the whole modal.
        return hero
      }
    }
