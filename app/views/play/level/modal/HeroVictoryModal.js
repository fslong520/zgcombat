...
module.exports = (HeroVictoryModal = (function () {
  HeroVictoryModal = class HeroVictoryModal extends ModalView {
    ...
    onAchievementsLoaded () {
      ...
      for (const thangTypeOriginal of Array.from(thangTypeOriginals)) {
    ...
    getRenderData () {
      ...
      for (achievement of Array.from(c.achievements)) {
        ...
        if (!this.supermodel.finished() || !achievement.get('proportionalTo')) { continue }
      ...
      if (me.get('hourOfCode')) {
        ...
        const hocCampaignSlug = application.getHocCampaign() && (application.getHocCampaign() !== 'intro')
        const lastLevelOriginal = (() => {
          switch (hocCampaignSlug) {
            ...
            case 'ai-league-hoc': return '62f9f6506428860025b15a8b' // magma-mountain; can change, but we will use isType('ladder') to cover that
              // when 'goblins-hoc' then '' # jungle-hunt
            default: return '541c9a30c6362edfb0f34479' // kithgard gates for dungeon
          }
        })()
        const lastLevel = (this.level.get('original') === lastLevelOriginal) || this.level.isType('ladder')
        ...
        this.showAmazonHocButton = (hocCampaignSlug === 'game-dev-hoc') && lastLevel
        if (this.showAmazonHocButton) {
          this.trackAwsButtonShown()
        }
        this.showHoc2016ExploreButton = hocCampaignSlug && lastLevel
    ...
    afterRender () {
      super.afterRender()
      this.$el.toggleClass('with-achievements', this.level.isType('hero', 'hero-ladder', 'game-dev', 'web-dev', 'ladder'))
    ...
    initializeAnimations () {
      if (!this.level.isType('hero', 'hero-ladder', 'game-dev', 'web-dev', 'ladder')) { return this.endSequentialAnimations() }
      this.updateXPBars(0)
    ...
    beginSequentialAnimations () {
      ...
      if (!this.level.isType('hero', 'hero-ladder', 'game-dev', 'web-dev', 'ladder')) { return }
    ...
    tickSequentialAnimation () {
      ...
      if (panel.unit === 'xp') {
        ...
        panel.textEl.text(utils.i18n(thangType.attributes, 'name'))
        if (ratio > 0.5 && ratio < 0.6) { this.playSelectionSound(thangType) }
      }
      if (ratio === 1) {
    ...
    updateXPBars (achievedXP) {
      ...
      this.displayedLevel = currentLevel
    }
    ...
    playSelectionSound (hero, preload) {
      ...
      const sounds = __guard__(hero.get('soundTriggers'), x => x.selected)
      if (!sounds) { return }
    ...
    getNextLevelCampaign () {
      ...
      if (Array.from(campaignEndLevels).includes(this.level.get('slug'))) {
