...
module.exports = class PlayLevelView extends RootView
  ...
  onLevelLoaded: (e) ->
    return if @destroyed
    # Allow students to play public levels (e.g. dungeon) without a course, like anonymous users
    if me.isTeacher() and not @courseID and not e.level.isType('course-ladder')
  ...
  onSessionLoaded: (e) ->
    return if @session
    Backbone.Mediator.publish "ipad:language-chosen", language: e.session.get('codeLanguage') ? "cpp"
