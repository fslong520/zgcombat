...
module.exports = (SystemsTabView = (function () {
  SystemsTabView = class SystemsTabView extends CocoView {
    ...
    constructor (options) {
      ...
      for (const system of Array.from(this.buildDefaultSystems())) {
        const url = `/db/level.system/${system.original}/version/${system.majorVersion}`
        const ls = new LevelSystem().setURL(url)
    ...
    getSortedByName (systems) {
      ...
      for (const sys of Array.from(systemModels)) { systemModelMap[sys.get('original')] = sys.get('name') }
