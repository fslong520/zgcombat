...
module.exports = (ThangComponentsEditView = (function () {
  ThangComponentsEditView = class ThangComponentsEditView extends CocoView {
    ...
    loadComponents (components) {
      return (() => {
        const result = []
        for (const componentRef of Array.from(components)) {
        // just to handle if ever somehow the same component is loaded twice, through bad data and alike
          ...
          const levelComponent = new LevelComponent(componentRef)
          const url = `/db/level.component/${componentRef.original}/version/${componentRef.majorVersion}`
          levelComponent.setURL(url)
          const resource = this.supermodel.loadModel(levelComponent)
    ...
    buildComponentsTreema () {
      ...
      const components = _.zipObject(((() => {
        const result = []
        for (c of Array.from(this.components)) {
      ...
      if (thangTypeComponents = this.thangType != null ? this.thangType.get('components', true) : undefined) {
        defaultValue = _.zipObject(((() => {
          const result1 = []
          for (c of Array.from(thangTypeComponents)) {
    ...
    onComponentsRemoved () {
      ...
      if (thangTypeComponents = this.thangType != null ? this.thangType.get('components') : undefined) {
        for (const thangTypeComponent of Array.from(thangTypeComponents)) {
      ...
      for (const subview of Array.from(_.values(this.subviews))) {
    ...
    onComponentsAdded () {
      ...
      while (true) {
        ...
        for (const componentRef of Array.from(_.values(componentMap))) {
          ...
          for (const dependency of Array.from((componentModel != null ? componentModel.get('dependencies') : undefined) || [])) {
      ...
      if (thangTypeComponents = this.thangType != null ? this.thangType.get('components') : undefined) {
        ...
        for (const thangTypeComponent of Array.from(thangTypeComponents)) {
          if (componentRef = componentRefs[thangTypeComponent.original]) {
            ...
            const modifiedRef = _.merge({}, thangTypeComponent)
    ...
    onConfigChanged (e) {
      ...
      for (const thangComponent of Array.from(this.components)) {
    ...
    selectKeyComponent () {
      return (() => {
        const result = []
        for (const child of Array.from(_.values(this.componentsTreema.childrenTreemas))) {
          var needle
    ...
    onSelectComponent (e, nodes) {
      ...
      while (componentsToCheck.length) {
        const componentOriginal = componentsToCheck.pop()
        for (const otherComponentRef of Array.from(this.components)) {
          if (otherComponentRef.original === componentOriginal) { continue }
          if (dependents[otherComponentRef.original]) { continue }
          const otherComponent = this.supermodel.getModelByOriginal(LevelComponent, otherComponentRef.original)
          for (const dependency of Array.from(otherComponent.get('dependencies', true))) {
    ...
    checkForMissingSystems () {
      ...
      const componentModels = ((() => {
        const result1 = []
        for (c of Array.from(this.components)) {
          result1.push(this.supermodel.getModelByOriginal(LevelComponent, c.original))
        }
        return result1
      })())
      const componentSystems = ((() => {
        const result2 = []
        for (c of Array.from(componentModels)) {
          if (c) {
      ...
      return (() => {
        const result3 = []
        for (const system of Array.from(componentSystems)) {
          if ((system !== 'misc') && !Array.from(extantSystems).includes(system)) {
            const s = `Component requires system <strong>${system}</strong> which is currently not included in this level.`
            result3.push(noty({
              text: s,
              layout: 'bottomLeft',
              type: 'warning'
    ...
    onAddComponentsButtonClicked () {
      let c
      const modal = new AddThangComponentsModal({
        skipOriginals: ((() => {
          const result = []
          for (c of Array.from(this.components)) {
            result.push(c.original)
      ...
      return this.listenToOnce(modal, 'hidden', function () {
        ...
        const sparseComponents = ((() => {
          const result1 = []
          for (c of Array.from(componentsToAdd)) {
            result1.push({ original: c.get('original'), majorVersion: c.get('version').major })
          }
          return result1
        })())
    ...
    onThangTypeKindChanged (e) {
      ...
      return (() => {
        const result = []
        for (const component of Array.from(defaultComponents)) {
