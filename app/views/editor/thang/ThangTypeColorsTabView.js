...
module.exports = (ThangTypeColorsTabView = (function () {
  ThangTypeColorsTabView = class ThangTypeColorsTabView extends CocoView {
    ...
    getColorConfig () {
      ...
      const tintMap = {}
      for (const tint of Array.from(this.tintAssignments.data)) {
        tintMap[tint.name] = tint
      }

      for (const k in this.tintedColorChoices) {
        const v = this.tintedColorChoices[k]
    ...
    createShapeButtons () {
      ...
      for (const color of Array.from(colors)) {
        const button = $('<button></button>').addClass('btn')
