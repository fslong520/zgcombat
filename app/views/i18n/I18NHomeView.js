...
module.exports = (I18NHomeView = (function () {
  I18NHomeView = class I18NHomeView extends RootView {
    ...
    onCollectionSynced (collection) {
      for (const model of Array.from(collection.models)) {
        model.i18nURLBase = (() => {
    ...
    getRenderData () {
      ...
      const covered = ((() => {
        const result = []
        for (m of Array.from(this.filteredModels.models)) {
    ...
    updateCoverage () {
      ...
      const relatedLanguages = ((() => {
        const result = []
        for (const l of Array.from(languages)) {
      ...
      for (const model of Array.from(this.filteredModels.models)) {
