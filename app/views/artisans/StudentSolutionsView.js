...
module.exports = (StudentSolutionsView = (function () {
  StudentSolutionsView = class StudentSolutionsView extends RootView {
    ...
    onLevelLoaded (level) {
      ...
      for (const solution of Array.from(level.getSolutions())) {
    ...
    getDefaultCode (level) {
      ...
      // non-javascript
      for (const key of Array.from(_.keys(programmableMethod.languages))) {
    ...
    walkAST (node, fn) {
      for (const key in node) {
        ...
        if (_.isArray(child)) {
          for (const grandchild of Array.from(child)) {
