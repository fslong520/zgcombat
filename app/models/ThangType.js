...
module.exports = (ThangType = (function () {
  ThangType = class ThangType extends CocoModel {
    ...
    getDefaultActions () {
      const actions = []
      for (const action of Array.from(_.values(this.getActions()))) {
    ...
    buildActions () {
      ...
      for (const name in this.actions) {
        ...
        for (const relatedName in object) {
    ...
    fillOptions (options) {
      ...
      if (options.resolutionFactor == null) { options.resolutionFactor = window.SPRITE_RESOLUTION_FACTOR }
    ...
    requiredRawAnimations () {
      ...
      for (const name in object) {
        ...
        for (const a of Array.from(allActions)) {
          if (a.animation) {
            const scale = name === 'portrait' ? a.scale || 1 : a.scale || this.get('scale') || 1
            const animation = { animation: a.animation, scale }
    ...
    uploadGenericPortrait (callback, src) {
      ...
      src = src.replace('data:image/png;base64,', '').replace(/\ /g, '+') // eslint-disable-line no-useless-escape
      const body = {
    ...
    static loadUniversalWizard () {
      ...
      this.wizardType = new module.exports() // eslint-disable-line new-cap
    ...
    getFrontFacingStats () {
      ...
      props.sort()
      const sortedStats = {}
      for (const key of Array.from(statKeys)) { sortedStats[key] = stats[key] }
      return { props, stats: sortedStats }
    }

    // Whether this ThangType is an actual equipping/moving/programmable hero.
    // Mirrors the checks in getHeroStats so callers can skip non-heroes silently.
    isRealHero () {
      const heroClass = this.get('heroClass')
      if (!heroClass) { return false }
      if (this.get('kind') === 'Junior Hero') { return false }
      const components = this.get('components') || []
      const equipsConfig = components.find(c => c.original === LevelComponent.EquipsID)?.config
      const movesConfig = components.find(c => c.original === LevelComponent.MovesID)?.config
      const programmableConfig = components.find(c => LevelComponent.ProgrammableIDs.includes(c.original))?.config
      return Boolean(equipsConfig && movesConfig && programmableConfig)
    }

    // Whether this ThangType is an actual equippable item.
    isRealItem () {
      const components = this.get('components') || []
      return Boolean(components.find(c => c.original === LevelComponent.ItemID))
    }
    ...
    isSilhouettedItem () {
      if ((this.get('gems') == null) && (this.get('tier') == null)) { return false }
    ...
    getContainersForActions (actionNames) {
      ...
      for (const actionName of Array.from(actionNames)) {
        ...
        if (action.container) {
          ...
          for (const container of Array.from(animationContainers)) { containersToRender[container.gn] = true }
        }
      }
    ...
    initPrerenderedSpriteSheets () {
      const data = this.get('prerenderedSpriteSheetData')
      if (this.prerenderedSpriteSheets || !data) { return }
      // creates a collection of prerendered sprite sheets
      this.prerenderedSpriteSheets = new PrerenderedSpriteSheets(data)
    }
    ...
    getRasterAtlasSpriteData (action) {
      ...
      const splitIndex = action.indexOf('_')
      if (!animation && (splitIndex !== -1)) {
        let relatedAction;
        [action, relatedAction] = Array.from([action.slice(0, splitIndex), action.slice(splitIndex + 1)])
      ...
      ssMetadata = ssMetadata.map(metadata => {
        const imgIndex = images.map(img => img.src)
          .map(imgSrc => imgSrc.split('/').pop())
          .map(imgName => imgName.slice(0, -4))
          .indexOf(encodeURIComponent(metadata.name))
        metadata.images = [images[imgIndex]]
        return metadata
      })
    ...
    onLoaded () {
      ...
      const itemConfig = __guard__(_.find(components, { original: LevelComponent.ItemID }), x => x.config)
      const equipsConfig = __guard__(_.find(components, { original: LevelComponent.EquipsID }), x4 => x4.config)
      if (itemConfig) {
        stats.kind = 'item'
        const speed = __guard__(itemConfig.stats != null ? itemConfig.stats.maxSpeed : undefined, x1 => x1.addend)
        const health = __guard__(itemConfig.stats != null ? itemConfig.stats.maxHealth : undefined, x2 => x2.addend)
        const attacksConfig = __guard__(_.find(components, { original: LevelComponent.AttacksID }), x3 => x3.config)
        ...
        const movesConfig = __guard__(_.find(components, { original: LevelComponent.MovesID }), x5 => x5.config)
        const attackableConfig = __guard__(_.find(components, { original: LevelComponent.AttackableID }), x6 => x6.config)
        if (movesConfig) {
    ...
    static formatStatsForHeroConfig (heroConfigStats, callback) {
      ...
      for (const stats of Array.from(heroConfigStatValues)) {
...
class PrerenderedSpriteSheet extends CocoModel {
  ...
  markToLoad () { this.needToLoad = true }
}
