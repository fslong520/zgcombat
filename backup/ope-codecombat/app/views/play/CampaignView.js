...
class CampaignView extends RootView {
  ...
  // Mirrors app/views/play/CampaignView.coffee so the shared campaign-view.jade
  // template (which calls view.showAds()) resolves correctly under the .js path.
  showAds () {
  ...
  onClickStartLevel (e) {
    ...
    const levelElement = btn.closest('.level-info-container')
    const levelDiv = btn.closest('.level')
    const levelSlug = levelElement.data('level-slug') || levelDiv.data('level-slug')
    const levelOriginal = levelElement.data('level-original') || levelDiv.data('level-original')
    const levelPath = levelElement.data('level-path') || levelDiv.data('level-path')
    const levelName = levelElement.data('level-name') || levelDiv.data('level-name')
    const level = _.find(_.values(this.getLevels()), { slug: levelSlug })

    // Local play: the stubbed backend has no real account system, so don't block
    // anonymous users behind a signup prompt that would dead-end. (Paid/teacher
    // gates were already removed for this offline clone.)
  ...
  checkForUnearnedAchievements () {
    if (!this.campaign || !globalVar.currentView || !globalVar.currentView.sessions) { return }
