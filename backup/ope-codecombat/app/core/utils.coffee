...
filterMarkdownCodeLanguages = (text, language) ->
  ...
  excludedLanguages = _.without ['javascript', 'python', 'cpp', 'coffeescript', 'clojure', 'lua', 'java', 'io', 'html'], currentLanguage
...
# --- decaffeinate-contract exports expected by .vue components ---
# These mirror the auto-decaffeinated app/core/utils.js so the newer Vue SFCs
# (which `import { cocoBaseURL, ... } from 'core/utils'`) resolve against the
# original .coffee source that webpack prefers.
CODECOMBAT = 'codecombat'
CODECOMBAT_CHINA = 'koudashijie'
OZARIA = 'ozaria'
OZARIA_CHINA = 'aojiarui'
isCodeCombat = COCO_PRODUCT is 'codecombat'
isOzaria = not isCodeCombat

cocoBaseURL = ->
  if isCodeCombat
    return ''
  if not require('./application').isProduction()
...
# The modern Vue SFCs (Footer.vue, Navigation.vue, etc.) were authored against
# the bulk-decaffeinate translation `app/core/utils.js`, which carries newer
# exports (`getJuniorUrl`, `isChinaOldBrowser`, `markedInline`, `AILeagueSeasons`,
# `orderedCourseIDs`, ...) this legacy `.coffee` source lacks. Merge them in.
# This block runs AFTER `module.exports` is assigned, so when `utils.js`'s eval
# transitively pulls `application` -> SystemNameLoader -> CocoClass -> `utils`,
# this module is already fully cached (not `{}`), which avoids the
# `combineAncestralObject is not a function` circular-dependency crash. Existing
# keys keep the canonical `.coffee` implementation.
try
  decafUtils = require './utils.js'
  if decafUtils and typeof decafUtils is 'object'
    for own k, v of decafUtils
      module.exports[k] = v unless module.exports.hasOwnProperty(k)
catch e
  console?.error?('core/utils: failed to merge decaffeinate exports:', e)
