...
module.exports = ModuleLoader = class ModuleLoader extends CocoClass
  ...
  load: (path, first=true, why) ->
    ...
    originalPath = path
    wad = _.find ModuleLoader.WADS, (w) -> _.string.startsWith(path, w)
    path = wad if wad
    return false if @loaded[path]
    if wad
      console.log "Loading", wad, " for ", originalPath if LOG

    # Under webpack there are no Brunch-style concatenated WAD files
    # (e.g. /dev/javascripts/app/views/play.js). The individual modules for
    # these directories are already bundled by webpack via require.context in
    # app.js, so there is nothing to fetch — just mark it loaded and let the
    # app proceed once the load queue drains.
    if wad and @wadModulesBundled(wad)
      @loaded[path] = true
      @_checkComplete()
      return true
    ...
    srcUrl = "/#{window.serverConfig.buildInfo.sha}#{uri}"
    console.debug 'Loading js file:', uri, "src:", srcUrl, "because", why if LOG
    @queue.loadFile({
      id: path
      src: srcUrl
  ...
  onFileLoad: (e) =>
    ...
    # Skip files that aren't valid JavaScript (e.g. 404 HTML pages)
    rawResult = e.rawResult?.toString() or ''
    if /^\s*(<!DOCTYPE|<html)/i.test(rawResult)
      console.warn 'ModuleLoader: skipping non-JS content for', e.item.id
      $(e.result).remove() if e.result
      @_checkComplete()
      return
...
#      console.debug 'loaded', @recentPaths.length, 'files,', parseInt(@recentLoadedBytes/1024), 'KB'
  ...
  onLoadError: (e) =>
    console.warn('ModuleLoader load error:', e?.message)
    @_inFlight = Math.max(0, @_inFlight - 1)
    @_checkComplete()

  # True when the individual modules under a WAD directory are already present
  # in the webpack bundle (pulled in by require.context in app.js).
  wadModulesBundled: (wad) ->
    names = window.require.list() or []
    return _.some names, (name) -> _.string.startsWith(name, wad + '/')

  # Fire 'load-complete' once, after the current synchronous stack and only when
  # no loads are still in flight. Deferring guarantees any listener attached by
  # the caller (e.g. Router's listenToOnce) is registered before we trigger.
  _checkComplete: ->
    _.defer =>
      if @_inFlight is 0 and not @_loadCompleteFired
