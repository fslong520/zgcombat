...
module.exports = class User extends CocoModel
  ...
  # Mirrors the bulk-decaffeinate translation (app/models/User.js) so the Vue
  # SFCs (e.g. Navigation.vue) that expect these methods resolve correctly.
  isInternal: ->
    email = @get('email')
    return false unless email
    email.endsWith('@codecombat.com') or email.endsWith('@ozaria.com')
  showOzCourses: ->
    return true if utils.isOzaria
    @isInternal() or @isAdmin() or @get('features')?.canAccessOzCourse
  # Mirrors app/models/User.js (bulk-decaffeinate) so the .js views that call
  # me.isMto() resolve correctly.
  isMtoStem: -> [utils.MTOClients?.MTO_STEM_DEV, utils.MTOClients?.MTO_STEM_PROD].includes(@get('clientCreator'))
  isMtoNeo: -> [utils.MTOClients?.MTO_NEO_DEV, utils.MTOClients?.MTO_NEO_PROD].includes(@get('clientCreator'))
  isMtoCodingOlympiad: -> [utils.MTOClients?.MTO_CODING_OLYMPIAD_DEV, utils.MTOClients?.MTO_CODING_OLYMPIAD_PROD].includes(@get('clientCreator'))
  isMto: -> @isMtoStem() or @isMtoNeo() or @isMtoCodingOlympiad()
  ...
  ownsLevel: (levelOriginal) -> true  # offline clone: all levels playable
...
# Merge any methods from the bulk-decaffeinate translation (app/models/User.js)
# that this hand-written User.coffee does not already define, so the .js views
# that call me.<method>() resolve. Placed at the end so User is fully defined.
# NOTE: ES6 class prototype methods are non-enumerable, so we must use
# Object.getOwnPropertyNames rather than a for...in loop.
do ->
  try
    DecafUser = require './User.js'
    proto = DecafUser.prototype
    for k in Object.getOwnPropertyNames(proto)
      unless User.prototype.hasOwnProperty(k)
        User.prototype[k] = proto[k]
  catch e
    console?.error?('models/User: failed to merge decaffeinate methods:', e)
