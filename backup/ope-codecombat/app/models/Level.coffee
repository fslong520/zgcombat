...
module.exports = class Level extends CocoModel
  ...
  usesLevelHeroLadderEquipment: ->
    # For hero-ladder levels where we have configured Hero Placeholder inventory equipment,
    # we must have intended to use it instead of letting the player choose their hero/equipment.
    return false unless @isType('hero-ladder')
    levelThang = _.find(@get('thangs'), id: 'Hero Placeholder')
    return false unless levelThang
    equips = _.find(levelThang.components, original: LevelComponent.EquipsID)
    return Boolean(equips?.config?.inventory)

  usesSessionHeroThangType: ->
    return false if @isType('ladder', 'course-ladder', 'game-dev', 'web-dev')
    return false if @get('assessment') is 'open-ended'
    return false if @usesLevelHeroLadderEquipment()
    return true

  usesSessionHeroInventory: ->
    return false if utils.showOzaria()
    return false if @get('product', true) is 'codecombat-junior'
    return false if @isType('course') and not me.showHeroAndInventoryModalsToStudents()
    return false if @isType('course') and @isAssessment()
    return false if @get('assessment')
    return false unless @isType('course', 'hero', 'hero-ladder', 'hero-coop')
    return false if @usesLevelHeroLadderEquipment()
    return true
