  ...
  return {
    ...
    module: {
      ...
      rules: [
        ...
        // Legacy Brunch-era `.jade` templates: use the `jade` compiler via a compat
        // loader (lenient about jade `extends`/`block` and keeps `{{ }}` literal).
        { test: /\.jade$/,
          oneOf: [
            {
              resourceQuery: /^\?vue/,
            ...
            {
              use: { loader: path.resolve(__dirname, 'jade-compat-loader.js') }
            }
          ]
        },
        {
    ...
    resolve: {
      ...
      extensions: [
        '.web.coffee', '.web.js', '.coffee', '.js', '.jade', '.pug', '.sass', '.vue',
        `.${productSuffix}.coffee`, `.${productSuffix}.js`, `.${productSuffix}.jade`, `.${productSuffix}.pug`, `.${productSuffix}.sass`, `.${productSuffix}.vue`,  //, `.${productSuffix}.scss` ?
      ],
      alias: { // Replace Backbone's underscore with lodash
        ...
        // Brunch resolved `require('vendor/x')` against bower_components for bower packages,
        // but project-local vendor files live under ./vendor/scripts and ./vendor/styles.
        // Keep the bower alias but override the specific project subpaths first.
        'vendor/scripts': path.resolve(__dirname, 'vendor/scripts'),
        'vendor/styles': path.resolve(__dirname, 'vendor/styles'),
        'vendor': path.resolve(__dirname, 'bower_components'),
        // Built bower/vendor libs that are required as `vendor/<name>` (no subpath):
        // their prebuilt `.js` files live in bower_components (copied from the old build).
        'vendor/treema': path.resolve(__dirname, 'bower_components/treema/treema.js'),
        'vendor/d3': path.resolve(__dirname, 'bower_components/d3.js'),
        // bare `require('ace')` -> ace-builds entry
        'ace': path.resolve(__dirname, 'bower_components/ace-builds/src-noconflict/ace.js'),
        // aether: use the build-aether output (bower_components/aether/build/aether.js)
        'aether': path.resolve(__dirname, 'bower_components/aether/build/aether.js'),
        // demo/test-only requires that are not part of the production client
        'demo-app': false,
        'tests': false,
        // game-libraries is provided as a local CoffeeScript module (app/lib/game-libraries.coffee)
        // in this webpack build (it was a Brunch-registered module originally). Alias it so the
        // bare `require('game-libraries')` resolves to the local source.
        'game-libraries': path.resolve(__dirname, 'app/lib/game-libraries.coffee')
      },
      ...
      fallback: {
        ...
        // jade-compat-loader bundles the `jade` compiler into the client; it expects
        // these Node core modules, which we polyfill/stub for the browser.
    ...
    plugins: [
      ...
      new webpack.ProvidePlugin({ // So Bootstrap can use the global jQuery
        ...
        THREE: path.resolve(PWD, 'bower_components/three.js')
      }),
